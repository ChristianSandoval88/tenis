        ...
        try {
            ...
            for (Place table : m_aplaces) {
                if(atickets.contains(table.getId()))
                {
                    ...
                    SharedTicketInfo ticket = l.stream().filter(x->x.getId().equalsIgnoreCase(table.getId())).findFirst().orElse(null);
                    
                    String[] nameList = ticket.getName().split("-");
                    String buttonText = "";
                    ...
                    table.getButton().setText("<html>"+buttonText+"<html>");
                }
                else 
                {
                    table.setPeople(false);
                    table.getButton().setText(table.getName());
                }
            }
        } catch (BasicException e) {
            new MessageInf(e).show(this);
